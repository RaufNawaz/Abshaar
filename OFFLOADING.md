...
| **`torch` does not import in `.venv`** (torch 2.8.0 on Python 3.9; `ImportError: cannot import name 'fetch' from partially initialized module 'torch.utils.data._utils'`) | Blocks `build-index`, `sentence_transformers`, `chromadb` — i.e. the whole RAG half of the pipeline, including the Phase 2 gate and every baseline run. mlx-lm is unaffected. | Verified 2026-08-31 from `/tmp` (not a cwd artefact) and reproduced via `sentence_transformers`. Fix: `brew install python@3.12`, recreate `.venv`, reinstall `requirements.txt` + mlx-lm. Then make `ai-check` actually import each package instead of `importlib.util.find_spec` (`src/abshaar/ollama_client.py:44`), which is why this was recorded as healthy on 2026-08-16. |
