...
- **STANDING CONSTRAINT (Rauf, 2026-08-15): do not run the Ollama models (smoke test, baselines, `ask`, `augment-training-data`, `run-eval`, or any generation) until he explicitly says so.** Training (Phase 5) also waits because baselines must precede it.
- **Session wrapped up at Rauf's request (2026-08-15, end of session). All project processes were stopped; nothing is running.** Download state at wrap-up: qwen3:4b fully pulled; qwen3:8b partially downloaded (~1.4 GB of 5.2 GB in `~/.ollama`, resumes on the next `ollama pull qwen3:8b`); BGE-M3 fully downloaded (2.4 GB in `~/.cache/huggingface`), so `./scripts/abshaar.sh build-index` now needs only the few-minute embedding step; mlx-lm NOT installed (pip was interrupted twice by a transient DNS outage affecting CDN hosts — see runbook §7). To resume: start `ollama serve`, then follow `docs/17_training_runbook.md` from its §1 preconditions checklist.
