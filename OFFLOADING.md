...
- **2026-08-31 (Claude): two complete off-machine training bundles built and shipped-ready.** Rauf now has a Windows workstation (Threadripper, 128 GB RAM, NVIDIA RTX 6000-class GPU) that **wipes all data at sign-out**, and asked for a bundle for it and for Mac. `scripts/export_training_bundle.sh` now builds both from committed sources in `training/bundle_src/{windows,mac}/` into `training/dist/`: `abshaar_windows_bundle.zip` (244 KB) and `abshaar_mac_bundle.zip` (232 KB), each with a base64 `.txt` twin for email (mail filters block `.ps1`/`.cmd`/`.py` inside ZIPs). Five resumable stages each — bootstrap/train/merge-or-fuse+GGUF/generate/pack — with a mandatory "copy this off before you sign out" step. Full runbook: `docs/18_workstation_training_runbook.md`. **Nothing has been run on real hardware yet.**
- **Built the unrestricted ("maximum records") dataset and bundle, 2026-08-31 (Claude), on Rauf's instruction to "remove any self imposed blocks and unreviewed blocks" and raise the learning rate.**
  - **New `GenerationPolicy` in `dataset_gen.py`** with four flags, all defaulting to the previous conservative behaviour so nothing changes for a caller that does not ask: `include_reference_translations` (emits Rafat, skips the leak scan whose only job is to catch it), `include_witnesses` (consumes the Sufinama records, whose crosswalk classifications remain `human_confirmed: false` — accepted deliberately), `witness_pairs_per_text` (cap on cross-view pairs), `holdout`. `GenerationPolicy.unrestricted()` sets the first two. **The holdout is deliberately NOT part of "unrestricted"** — a test asserts this — because training on the eval split destroys the ability to measure anything; `--no-holdout` exists for a final production run after a recipe is validated.
  - **Two new families.** `reference_translation` (63 train / 8 eval) uses its own question wording rather than competing with the drafted `translation` family for the same prompt — two answers to one question is contradictory signal and would trip the dedup gate. `script_conversion` (462/54) and `witness_identification` (13/2) come from the witnesses. Verified before building: all 79 multi-view witness texts are line-aligned across views (zero line-count mismatches), so cross-view pairs are genuine parallel data, not an assumption. 124 witness texts, 516 ordered pairs available.
  - **Result: 1,783 examples (1,576 train / 207 eval) vs 1,181 (1,038/143) — +51%.** `script_conversion` is 29% of it; `--witness-pairs 2` gives a less skewed 1,425 total. Written to `data/processed/training_max/` via the new `--out-dir`, so the in-flight run's dataset was NOT clobbered (verified: `data/processed/training/mlx/train.jsonl` still hashes `c8aa7676…`).
  - **Probes are per-dataset now.** `build_probes(root, training_dir)` writes into the dataset's own directory, because probes drawn from another build's eval split would be questions the model was trained on. `data/processed/training_max/probes.jsonl` is its own 50.
  - **Warm-start support (2026-08-31):** `01_train.sh` gained `-t/--tag` (gives a run its own folder under `runs/<slug>-<tag>/`) and `--init-from <adapter>` (warm-starts a NEW run from another run's weights). Both were needed because mlx-lm restarts the iteration counter at 1, so an untagged second run would overwrite the very checkpoints it was warm-starting from; the script now refuses `--init-from` pointing at its own output directory. Only adapter weights carry over — optimizer state and the iteration counter reset — so a warm start is a new run, not a continuation, and should be reported that way. Verified against a stub python: correct `--resume-adapter-file`/`--adapter-path` pairing and the self-overwrite guard both fire.
  - **Learning rate raised 1e-5 → 1e-4** in the Mac bundle (`01_train.sh`), with `--lr` passthrough added to `run_all.sh`. Recorded in `train_summary.json`. Documented plainly: a higher rate takes *bigger* steps, not faster ones — wall clock is iterations × seconds-per-iteration, so the saving only arrives if `-i` drops too (800–1000 suggested against 1560, to be confirmed on the validation curve, not assumed).
  - **Still excluded, and why:** the 160 PunjabLibrary Gurmukhi witnesses. Not a policy gate — the extracted text is documented-defective (missing/misordered characters; page images are authoritative) and they are not in the knowledge base at all. Including known-corrupt text would teach corruption. Adding them means fixing the extraction or human-correcting it first, then a KB integration; flagged rather than silently included or silently dropped.
  - **Verification:** 89 tests pass (6 new, including a regression guard that the 8-gram leak gate still fires when reference text is *not* invited — the opt-in must not have disabled it). `validate` clean. Bundles rebuilt: `abshaar_mac-max_bundle.zip` (680 KB, 1,576/207) alongside the unchanged `abshaar_mac_bundle.zip` (1,038/143).
  - **Files affected:** `src/abshaar/{dataset_gen,training_export,knowledge_base,evaluate,cli}.py`, `tests/test_dataset_gen.py`, `scripts/export_training_bundle.sh` (`--data-dir`, `--suffix`), `training/bundle_src/mac/{01_train.sh,run_all.sh,README_MAC.md}`, `docs/19_mac_training_walkthrough.md`, new `data/processed/training_max/`.
- **Audited what the 1,038 training examples actually cover, 2026-08-31 (Claude), after Rauf asked whether the model trains on the whole accumulated corpus. Answer: no, and mostly by design.** Measured from the live files:
  - **In training:** all 72 poems' trainable layers (360 records: 72 original, 72 transliteration, 72 literary_translation, 72 tashreeh, 71 ai_translation, 1 literal_gloss) plus 321 term, 297 theme, 11 biographical_claim and 15 cluster_relation KB records. Families: term 281, theme 261, transliteration 126, identification 126, honesty 89, translation 63, tashreeh 63, biography 16, variant_awareness 13.
  - **Deliberately excluded:** Rafat's 71 reference translations (`trainable: false`, copyright gate — note only **1** genuine `literal_gloss` exists, the rest of that slot is Rafat's text, which is the schema mismatch already logged); 8 work clusters held out for evaluation (`work_bulleh_shah_{0001,0011,0021,0032,0042,0052,0062,0072}`, leaving 5 poems absent from training entirely); the 160 private PunjabLibrary Gurmukhi witnesses (defective OCR, not in the KB).
  - **In the RAG knowledge base but NOT trained on:** the 289 Sufinama witness records (220 `sufinama_kaafi_witness` + 69 `sufinama_text_witness`). Only the 15 `cluster_relation` records derived from them become training examples (the `variant_awareness` family). KB is 1,306 records vs 1,038 training examples — the plan's division of labour: LoRA teaches form and honesty, RAG supplies facts at query time. This is why acceptance is measured tuned+RAG vs base+RAG.
  - **Verified clean:** train and eval canonical work clusters do not overlap at all.
  - **Finding — mild cross-cluster exposure.** `theme`/`term` examples enumerate poem titles across clusters, so a training example about `work_bulleh_shah_0031` names "Sab iko rang kapaahaan da" (`bulleh_shah_0032`, a held-out work). The model sees held-out poems' **titles**, never their text, translation or tashreeh. Probably acceptable, but it means the split is cluster-disjoint at the level of *content*, not of *mention* — state it that way rather than claiming a perfectly clean split.
  - **Finding — 63 unresolvable provenance ids.** Every `tashreeh`-family example cites `kb_ids` like `tash_bulleh_shah_0002_beginner` while the KB record is `kb:tash_bulleh_shah_0002_beginner`; the `kb:` prefix is dropped for that family only (all other families resolve). The records exist, so nothing is missing and **nothing reaches the model** — `data/processed/training/mlx/train.jsonl` carries only `messages`, no metadata — but any future citation-resolution check will fail on those 63. Fix in `dataset_gen.py`, then re-export; do NOT do it while a training run is in flight, because it changes `train.jsonl`'s SHA-256, which that run's `train_summary.json` records. RULE 4 follow-up: add a test asserting every `kb_id` in the generated dataset resolves against `knowledge_base.jsonl`.
- **Run 1 imported, and three defects found in its configuration (2026-08-31).** `run1_results.zip` (36 MB) verified with `shasum -c` (3/3 OK) and imported via `scripts/import_trained_adapter.sh` to `training/adapters/mlx-community_Qwen3-8B-4bit`, **sha256 `ec67f93d8224f1c83285eb76f13a8e98012ac41a8ca8b821f3858d67cdca7b17`**. Log preserved at `training/logs/run1-train-20260831-160232.log`. `training/EVAL_MATRIX.md` now holds the full run record; its acceptance rows are explicitly empty pending the Mac-side gate.
  - The adapter is 9.699 M params in **F32** (37 MB, not the 19 MB an fp16 estimate predicted).
  - The log shows only 3 checkpoint saves (200/400/600), so the run was stopped between the iteration-800 report and its save. `adapters.safetensors` was therefore already the iteration-600 state — Rauf's promotion step was a no-op, and the imported adapter is confirmed to be iteration 600.
  - **Corrected reading of the curve.** Individual val points are noisy (±0.1, random third of the set), so no single point proves overfitting — but train loss fell 0.969 → 0.390 while val never improved, widening the train/val gap 0.468 → 1.128. *That* is the overfitting evidence. Iterations 200 (1.389) and 600 (1.305) are statistically indistinguishable.
  - **Defect 1 — `mask_prompt: false`.** mlx-lm's default computes loss over the QUESTION as well as the answer, so run 1 was partly trained to generate prompts. The CUDA trainer already masks. `--mask-prompt` is now on by default in the Mac bundle (`--no-mask-prompt` restores the old behaviour).
  - **Defect 2 — `max_seq_length: 2048`.** The corpus's longest examples run to ~5,700 characters of Shahmukhi/Urdu, well past 2048 tokens, so their answers were truncated mid-poem — training the model to stop early. Default raised to 4096.
  - **Defect 3 — learning rate.** The 1e-4 default set earlier today at Rauf's request was reverted to 1e-5 the same day: run 1 disproved the underfitting premise it rested on (val hits its floor in ~200 iterations while train keeps falling). A larger rate reaches that floor sooner, not a better one. `--lr` still overrides.
- **Run 1 full validation curve, and what it actually showed (2026-08-31).** `Iter 1: 3.526 | 100: 1.437 | 200: 1.389 | 300: 1.528 | 400: 1.553 | 500: 1.458 | 600: 1.305 | 700: 1.313 | 800: 1.518`. Train loss at 600 was 0.811. Run stopped at ~800 on Claude's advice.
  - **94% of all learning happened by iteration 100** (3.526 → 1.437 of a total 3.526 → 1.305). After that the curve oscillates with no trend: min 1.305, max 1.553, range 0.248, stdev 0.095.
  - **Correction: the iteration-800 rise was NOT overfitting onset.** Claude called +15.6% at iter 800 "not noise" from three points; the full curve shows 1.518 sits *inside* the range the curve had occupied since iteration 100 — iteration 400 was worse at 1.553. Stopping was still correct (nothing improved after ~200), but the stated reason was wrong.
  - **Root cause of the oscillation, verified in the installed mlx-lm:** `trainer.py:128` does `indices = np.random.permutation(len(batch_idx))` and `evaluate()` consumes only the first `--val-batches` (default 25) of that permutation. So **every validation point scored a different random 50 of the 143 validation examples.** Batches are also grouped by length (`idx = sorted(..., key=len_fn)`), so the subsets differ systematically in difficulty. The curve was measuring sampling noise, not the model.
  - **Fix:** `01_train.sh` now computes `--val-batches` to cover the whole validation set (103 batches × 2 for the 207-example max set; empty `--val-batches` means auto). Recorded in `train_summary.json`. Cost: validation goes from ~55 s to roughly 2.5-4x that, so with `--eval-every 100` budget ~20 min of a run for a curve that can actually be read.
  - **Consequence for run 2:** this dataset saturates in ~100-200 iterations, so long runs buy nothing — the lever is more/more-diverse data (the max dataset, +52%), not more steps. And raising the learning rate to 1e-4 is contraindicated: 1e-5 already reaches the floor within one epoch.
- **Run 1 progress and two fixes it prompted (2026-08-31).** At iteration 600 of 1560: train loss 0.811, **val loss 1.305** (against 3.526 untrained at iter 1), 302,496 trained tokens, **peak memory 24.382 GB**. Learning is healthy — val down 63% — with a train/val gap of ~0.49 at ~1.16 epochs.
  - **Peak memory more than doubled mid-run** (11.0 GB at iter 25 → 24.4 GB at iter 600) because MLX's peak tracks the longest sequence seen so far and this dataset ranges from a couplet to ~2,900 tokens (417 → 504 tokens/iter). Consequence: peak memory must be read LATE in a run. A batch-size-4 note written off the early figure was wrong and has been corrected — batch 4 projects to ~45 GB, making it a 64 GB+ lever. It also confirms run 1 is not on the 16 GB Air.
  - **The final adapter is often not the best adapter.** mlx-lm's `adapters.safetensors` is the last state, not the minimum-validation one. Added `training/bundle_src/mac/best_checkpoint.py`: parses a training log, prints the whole validation curve with a trend column, names the minimum and the checkpoint file to use, and warns when that checkpoint was never saved. Verified on a synthetic log with a rising tail.
  - **Checkpoint cadence now matches validation cadence** (`--save-every` == `--steps-per-eval`, both 100, `--eval-every` changes them together). Previously 200 vs 100, so half the validation points had no recoverable checkpoint — a validation minimum you cannot restore is not usable. Recorded in `train_summary.json`.
  - **Consequence for packing:** if `best_checkpoint.py` names an earlier checkpoint, do NOT use `04_pack_outbox.sh --slim`, which drops exactly those files.
- **First real training run started 2026-08-31 (Rauf, on a Mac).** `mlx-community/Qwen3-8B-4bit`, batch 2, `-i 1560` (~3 epochs over 1,038 examples). Measured: 9.699 M trainable params of 8190.735 M (0.118%, mlx-lm's default 16 layers), 0.315 it/s ≈ 3.2 s/iter, ~131 tokens/s, **peak memory 11.0 GB**, validation ~65 s every 100 iters. Iter 1 val loss 3.526 (untrained baseline) → iter 25 train loss 2.435. Projected wall clock ≈ 100 min. mlx-lm's default learning rate is 1e-5 (the CUDA bundle uses 2e-4) — conservative for LoRA; the first lever if the final val loss plateaus high, ahead of more iterations. These are the project's first measured training numbers; recorded in `docs/19` §3.3.
- **Blocker found 2026-08-31: `torch` in `.venv` does not import** (torch 2.8.0 in a Python 3.9 venv → circular-import failure), so `build-index` and every `sentence_transformers`/`chromadb` path will fail until the venv is rebuilt on Python 3.11/3.12. `ai-check` reports it as installed because it uses `find_spec` (never imports). mlx-lm is unaffected. See §9.
- **Session end-of-day state (2026-08-16, Rauf about to close this session and next pick up work at a Harvard Mac Studio workstation).** Two ways forward now exist for the model-dependent work, built this session and NOT yet run — see the §7 Urgent item for the decision:
  1. **Stay local (this Air):** `./scripts/thermal_aware_pipeline.sh status` (safe, no side effects) → `touch training/RUN_AUTHORIZED` → `./scripts/thermal_aware_pipeline.sh run`. Runs Phases 2-5 at background priority, pauses automatically if `pmset -g therm` reports thermal throttling.
  2. **Move to a Mac Studio:** `training/portable_bundle/` already exists (built + tested live this session; dataset unchanged since, so it's current) — copy that folder to the Mac Studio, run `./train_standalone.sh` there, bring the resulting `adapters/<model>/` folder back, then `./scripts/import_trained_adapter.sh <path>` on this Mac. Full detail in `training/README.md` §"Path C" and `docs/17_training_runbook.md` §5.
  - Either way, `build-index` (Chroma/BGE-M3 embedding) still needs to run once on THIS Mac before the RAG smoke test — that step doesn't move to the Mac Studio, it's what the Mac Studio path trains against downstream.
  - Local AI stack is fully ready: `qwen3:4b` + `qwen3:8b` pulled, mlx-lm installed and import-verified, BGE-M3 cached, `ollama serve` running.
  - **Git: `git push origin draft` was blocked for Claude by the auto-mode permission classifier on 2026-08-16, but succeeded normally on 2026-08-31** (`906d952..60a9922`), so treat that block as environment-specific and no longer assumed — it is not a problem with the commits. Rauf pushed everything through his own `!` command earlier in the session and will need to do the same for the final commit each time; see the exact command in §7.
...
- Built two complete off-machine training bundles (Windows/CUDA and Apple Silicon) on 2026-08-31 (Claude), after Rauf said he now has a Windows workstation (Threadripper, 128 GB RAM, NVIDIA RTX 6000-class GPU) **that deletes all data when he signs out**, asked for a script that downloads everything needed onto it, and then asked for the same for Mac.
  - Details: New committed sources in `training/bundle_src/{windows,mac}/`, built into shippable bundles by a rewritten `scripts/export_training_bundle.sh` (outputs to gitignored `training/dist/`). Each bundle is five numbered stages plus a one-command orchestrator: bootstrap (downloads everything), train, merge/fuse + GGUF export, base-vs-tuned generations, pack-the-outbox. **Windows/CUDA path (new, Path D):** `train_lora_cuda.py` is plain `transformers.Trainer` + `peft` — deliberately no Axolotl, TRL, DeepSpeed or bitsandbytes, all of which break on Windows and none of which is needed for a bf16 LoRA on 8B in 48 GB of VRAM; loss is masked to the assistant turn by longest-common-prefix between templated prompt and templated full conversation; `attn_implementation="sdpa"` (no flash-attn wheels on Windows); torch installs from `--index-url .../whl/cu128` (cu128 covers both Ada sm_89 and Blackwell sm_120), never `--extra-index-url`, because the default PyPI Windows torch wheel is CPU-only and pip prefers it; `check_env.py` exits non-zero if CUDA ends up unavailable. `00_bootstrap.ps1` installs a private Python 3.12 under the work root if none is usable (user-scope, no admin). `02_merge_export.ps1` merges the adapter and exports a Q4_K_M GGUF via llama.cpp (best-effort; failure is reported, not hidden), plus an Ollama `Modelfile`. **Mac path (Path C, rebuilt):** the same five stages over mlx-lm, replacing the earlier single `train_standalone.sh`; `02_fuse_export.sh` detects a quantised fuse and explains that GGUF needs a bf16 base rather than failing obscurely.
  - Ephemeral-machine design (the workstation wipes at sign-out): everything — venv, HF cache, checkpoints, results — lives under one `-Root`/`-r` folder, never the user profile; every stage drops a marker in `<root>/.stages` so an interrupted run resumes; training checkpoints every 50 steps (CUDA) / 200 iters (MLX) and both accept `--resume`; the final stage writes `outbox/`, `SHA256SUMS.txt` and a small ZIP, then prints an ordered "copy this off before you sign out" checklist with real sizes; pointing the root at an external drive preserves the ~20 GB of downloads between sessions.
  - Shipping: `training/dist/` holds `abshaar_{mac,windows}_bundle.zip` (232 KB / 244 KB) **and a base64 `.zip.b64.txt` copy of each**, because Gmail and most corporate filters block `.ps1`/`.cmd`/`.py` attachments *inside archives* and reject the whole message; `training/dist/EMAIL_ME.md` gives the decode command per platform (`certutil -decode` on Windows, `base64 -D` on macOS).
  - Verification: `scripts/export_training_bundle.sh` run live — both bundles built, zipped, base64-encoded, SHA-256 printed; MANIFESTs carry the correct counts (1038/143/143/50) and hashes. `bash -n` on all 12 shell scripts, `py_compile` on all 6 Python scripts. The prompt-masking algorithm was verified against **all 1,038 real training rows** with a stub tokenizer reproducing Qwen3's non-thinking template: the supervised span equals the assistant content plus `<|im_end|>` in every row, the masked prefix ends exactly at the assistant header, an over-long example is dropped rather than producing an all-`-100` batch, and the `TypeError` fallback for tokenizers that reject `enable_thinking` works. **Not verified: nothing has been run on real hardware** — no CUDA machine and no second Mac was available in this session.
  - Finding (2026-08-31, unrelated to the bundles but blocking Mac-side work): **`torch` in `.venv` does not import.** `import torch` fails with `ImportError: cannot import name 'fetch' from partially initialized module 'torch.utils.data._utils' (most likely due to a circular import)` — torch 2.8.0 installed into a Python 3.9 venv. `sentence_transformers` fails the same way, so `build-index` (Chroma/BGE-M3) and every RAG path will fail until the venv is rebuilt on Python 3.11/3.12. `ai-check` does not catch this because `src/abshaar/ollama_client.py:44` uses `importlib.util.find_spec`, which checks the package exists on disk and never imports it — which is why 2026-08-16 recorded all five optional packages as installed. mlx-lm is unaffected (it does not need torch).
  - Why it matters: the workstation is far more capable than this Air *and* far more perishable. The bundles make a session on it a single command with a mandatory exfiltration step, and Path D is the only path that returns an Ollama-servable GGUF without a second heavy machine.
  - Files affected: `training/bundle_src/windows/{common.ps1,00_bootstrap.ps1,01_train.ps1,02_merge_export.ps1,03_generate.ps1,04_pack_outbox.ps1,RUN_ALL.ps1,RUN_ALL.cmd,train_lora_cuda.py,merge_adapter.py,generate_outputs.py,check_env.py,download_model.py,requirements-cuda.txt,README_WINDOWS.md}` (new), `training/bundle_src/mac/{common.sh,00_bootstrap.sh,01_train.sh,02_fuse_export.sh,03_generate.sh,04_pack_outbox.sh,run_all.sh,generate_outputs_mlx.py,requirements-mlx.txt,README_MAC.md}` (new), `scripts/export_training_bundle.sh` (rewritten; legacy positional form still accepted), `scripts/import_trained_adapter.sh` (now accepts peft `adapter_model.safetensors` as well as mlx `adapters.safetensors`), `docs/18_workstation_training_runbook.md` (new), `docs/19_mac_training_walkthrough.md` (new — exact command-by-command Mac walkthrough, incl. the outstanding acceptance gate), `docs/17_training_runbook.md` §5, `training/README.md`, `.gitignore` (`training/dist/`).
...
- `./scripts/export_training_bundle.sh [--target mac|windows|both]` (builds the off-machine training bundles into `training/dist/`; no model is touched)
...
- [ ] Decide which training path to use next — four now exist, none yet run: **A** local M4 (`scripts/thermal_aware_pipeline.sh`, background-friendly, thermally paced), **B** rented cloud GPU (`training/axolotl_qwen3_8b.yml`), **C** another Apple Silicon Mac, **D** the Windows/NVIDIA workstation. C and D ship as bundles from `./scripts/export_training_bundle.sh` (see `docs/18_workstation_training_runbook.md`). **Recommendation: D**, because it is the only path that returns an Ollama-servable Q4_K_M GGUF without a second heavy machine, and the workstation's 128 GB / RTX 6000-class GPU makes 8B bf16 LoRA a sub-hour job. Whichever path: Qwen3-8B is the grounded choice, since a `qwen3:8b` baseline is already planned in `docs/17` §3.
- **Run 2 complete and imported (2026-08-31).** Val `1.343 → 1.218 → 1.189 → 1.185 → 1.178` over 400 iterations, **monotonically decreasing at every point** — the whole-set validation fix turned an unreadable curve into a readable one. Adapter sha256 `66c1239724991c1c907cc2fa9a76a7f82427962dbfdf3ad8cfe70a8cff6fe85b`, imported to `training/adapters/mlx-community_Qwen3-8B-4bit-run2`; zip verified 5/5 checksums; log and summary in `training/logs/run2-*`.
  - **Run 2 beats run 1 by 12.3% like-for-like.** Iteration 1 (1.343) is run 1's adapter evaluated on run 2's validation set with run 2's masked loss, so the comparison is on identical terms. Run 2's eval split may contain material run 1 trained on, which would flatter run 1; it lost anyway.
  - `train_summary.json` confirms every fix reached the run: `mask_prompt: true`, `max_seq_length: 4096`, `val_batches: 103`, `save_every: 100 == steps_per_eval: 100`, lr 1e-5, dataset sha256 matching the max build exactly.
  - **Converged**: first 100 iterations delivered 76% of the gain, last 200 delivered 0.011. Final checkpoint is the best, so no promotion was needed. **A run 3 is not worth its wall clock** — the remaining lever is corpus work (docs/20), not more steps.
  - Cost 66 min: 36 training (5.4 s/iter, up from 3.2 because 4096 stops truncating long kafis) + 30 validating.
  - **Both adapters now live in the repo**; the trainer machine can be released.
- **Bug caught before it bit (2026-08-31): the post-training stages ignored `-t/--tag`.** `01_train.sh` gained tagged run folders so run 2 would not overwrite run 1, but `02_fuse_export.sh`, `03_generate.sh` and `04_pack_outbox.sh` all derived `SLUG` from the model alone — so fusing, generating or packing a tagged run would have silently operated on the UNTAGGED folder, i.e. packed run 1 and reported it as run 2. All three now accept `-t/--tag`, and `run_all.sh` passes it through. Verified with two adapter folders on disk (`RUN1`/`RUN2` sentinels): packing with `-t run2` picks run 2.
- **docs/20 steps 1-2 implemented, 2026-08-31 (Claude), while run 2 trained.**
  - **The review loop is closed.** New `src/abshaar/reviews.py` reads `data/annotations/reviews.jsonl` and turns `corrected_translation`/`corrected_tashreeh` into corpus layers with `created_by: human`, `reviewer`, `review_id` and `supersedes`. Corrections **replace** the AI layer rather than sitting beside it — a corpus emitting both teaches the model they are interchangeable. Wired into `training_export.extract_trainable_layers`, `knowledge_base.build_kb` and `dataset_gen.build_examples`; examples now carry `human_reviewed`, and the MANIFEST reports how many examples came from corrected layers. Placeholder detection is deliberately narrow (`^\[[^\]]*\]$`) so a real correction containing `[uncertain line — torn]` is not mistaken for an unfilled template field. 9 new tests in `tests/test_reviews.py`, including the invariant that a reviewed poem never emits the AI draft.
  - **Translation schema resolved.** `# Reference Translation` is now a first-class section (`markdown_entry.py`), and `abshaar migrate-reference-headings [--apply]` moved the 71 Rafat entries out of `# Literal Translation`. Verified: content SHA excluding the heading line is byte-identical before and after (`88e59750…`), record counts unchanged (72 ai_translation / 72 literary_translation / 71 reference_translation / 1 literal_gloss), 360 trainable layers, 1,306 KB records, validate clean. Empty slots now emit no record at all rather than a placeholder. The entry template gained the new section with a comment explaining the rights position.
  - **What this unblocks:** the five-poem gold slice can now be authored into the right fields, and a review written for it will actually change what the model trains on. That was not true this morning.
- [ ] **Work the corpus, not the training — `docs/20_corpus_next_steps.md` (written 2026-08-31).** The model's ceiling is the corpus: 217 of 288 interpretive layers are AI-authored, 0 human reviews exist, and run 1's validation floored at ~200 iterations because 1,038 examples come from ~10 templates. Ordered plan with hour estimates in docs/20. **Do steps 1-2 first (schema decision, then wiring the review loop) — they are code and take about a day**, and without them the five-poem gold slice is authored into the wrong fields and feeds nothing.
- [ ] **Finding that blocks the review work: nothing consumes `data/annotations/reviews.jsonl`.** It is counted (`status.py:22`) and schema-validated (`validation.py:181`) and that is all — `dataset_gen.py` has no notion of a human correction and no `created_by: human` preference over `ai`. Writing reviews today changes nothing about training. See docs/20 §1 for the two small changes and the test that closes the loop.
- [ ] **Rafat rights gate — Rauf stated 2026-08-31 that he has explicit permission to use the Taufiq Rafat translations, so the training exclusion can be lifted.** Recorded as his decision. Two things must accompany it before the code changes:
  1. **Write the permission down** in the Rafat source record in `data/context/sources.jsonl`: who granted it, when, and its scope (private model training? redistribution? publication?). The §9 risk table already flags the Sufinama authorization as "user-attested but no written reference is stored" — the same gap should not be repeated here, and text trained into weights cannot be removed later.
  2. **Keep the public-export gate.** `src/abshaar/export.py:22` strips `reference_translation` from the site export; permission to train a private model is not permission to publish, and these are separate switches. Only the training path should open.
  - Code changes when authorized (one commit): `markdown_entry.py:151` `trainable: False` → `True` for the reference slot; relax the leak scan in `training_export.py` (`find_leaks`, and the reference index built at `:71`), `knowledge_base.py:226-232`, `dataset_gen.py:415,449` — that scan exists specifically to catch this text, so it must become opt-out rather than being deleted. Then `generate-training-data` → `export-mlx-dataset`, which changes `train.jsonl`'s SHA-256 and therefore requires a fresh training run.
  - **Measured delta:** 71 `reference_translation` records, mean 657 chars, covering 71 of 72 poems. The gain is *quality of target*, not volume — every translation the model currently learns from is AI-drafted (`literary_translation`, 72 records, mean 784 chars; `ai_translation`, 72). Expect roughly +7-14% training examples. **This is not the bottleneck:** without `eval_baseline.md` there is no way to tell whether the addition helped, so baselines come first.
- [ ] **Rebuild `.venv` on Python 3.11/3.12** before any Mac-side model work: `torch` currently fails to import in the 3.9 venv, which blocks `build-index`, the RAG smoke test and every baseline run. Consider also making `ai-check` import each optional package instead of `find_spec`-ing it, so the status command stops reporting a broken install as healthy (RULE 4: encode the invariant).
- [ ] Run the acceptance gate that off-machine training does **not** remove (step-by-step, with the venv rebuild first, in `docs/19_mac_training_walkthrough.md` Part 6): `build-index` → RAG smoke test → the three baseline runs (`eval_baseline.md` does not exist yet) → only then compare the tuned model. Criterion unchanged: tuned+RAG ≥ base+RAG on factual AND tuned ≥ base on honesty.
...
Copy and paste this into a new chat or Codex session (on this Mac OR on a
Harvard Mac Studio — say which in your first message, since the two next
steps differ):

> I am continuing the Abshaar project, a local-first, human-reviewed archive and AI-assisted interpretation system for South Asian mystical poetry, currently building a private Bulleh Shah expert model (RAG + LoRA).
>
> First, read `AGENTS.md`, `CLAUDE.md`, `OFFLOADING.md` in full, and `Bulleh Shah/CORPUS_BUILD_LOG.md`. Then run `git status --short --branch` and inspect the diff for every file you may edit. Codex and Claude share this working tree, so preserve all pre-existing modified and untracked work.
>
> Current state (verified 2026-08-16): 72 draft Bulleh Shah entries with complete AI-drafted interpretive layers; 76 Sufinama kaafi + 48 non-kaafi witnesses, ALL 124 now AI-classified against the 72 entries (exact/variant/excerpt/possible/unmatched) with per-record line-coverage evidence, but `human_confirmed: false` on every one — treat these as a strong first pass, not ground truth, until Rauf confirms; 160 private PunjabLibrary Gurmukhi witnesses. Validation: 0 errors, 0 warnings. 83 tests pass. Hardware: Apple M4 MacBook Air, 16 GB RAM, fanless.
>
> The local AI stack is mostly ready: Ollama 0.32.13 with `qwen3:4b` + `qwen3:8b` pulled, mlx-lm 0.29.1 installed and import-verified, BGE-M3 cached. **But `torch` in `.venv` does not import (torch 2.8.0 in a Python 3.9 venv), so `build-index`, `sentence_transformers` and `chromadb` will fail until the venv is rebuilt on Python 3.11/3.12 — `ai-check` misses this because it uses `find_spec` and never imports.** **Standing instruction, still in force: do not run any Ollama/MLX model (smoke test, baselines, augmentation, training, or the `build-index` embedding step) until Rauf explicitly authorizes it in your session.** Two tools exist so this authorization is a deliberate, durable action rather than something to infer from chat tone:
> - `scripts/thermal_aware_pipeline.sh` (run docs/17 Phases 2-5 locally, at background priority, paused automatically under thermal throttling) refuses to touch a model unless `training/RUN_AUTHORIZED` exists — check `./scripts/thermal_aware_pipeline.sh status` first; it has zero side effects.
> - If you are on another machine — a Harvard Mac Studio, or Rauf's Windows workstation (Threadripper, 128 GB, NVIDIA RTX 6000-class, **wipes all data at sign-out**) — build the bundles on the Air with `./scripts/export_training_bundle.sh` and carry `training/dist/abshaar_{mac,windows}_bundle.zip` (or the `.zip.b64.txt` twin if emailing; mail filters block `.ps1`/`.cmd`/`.py` inside ZIPs). Each bundle is one command (`RUN_ALL.cmd` / `./run_all.sh`) covering bootstrap → train → merge/fuse + GGUF → generations → pack, all resumable, ending in a mandatory copy-off step. Bring back the adapter for `scripts/import_trained_adapter.sh` and the GGUF for `ollama create`. Full detail: **`docs/18_workstation_training_runbook.md`**, `training/README.md` §"Paths C and D".
>
> Immediate work: follow **`docs/17_training_runbook.md`** step by step regardless of which path — preconditions checklist, smoke test, baselines, optional augmentation, LoRA training, fuse/serve, acceptance criteria. Never start a step while the previous step's gate fails, and never weaken a gate. Separately, `data/annotations/crosswalk_classifications.jsonl` is open for Rauf's review/edits (start with the one `possible` record and the composite kaafi-44 page flagged in its notes) — re-run `abshaar apply-crosswalk-review` after any edit.
>
> Safety: never overwrite the 72 Markdown entries with Sufinama data; keep each source witness separate and preserve UUID/line/token provenance. The repo is private on GitHub (`origin/draft`) — keep it that way; it contains Rafat's copyrighted reference translations. **Note: an automated permission classifier in this harness blocks `git push` for the assistant — Rauf runs pushes himself via `! git push origin draft`.**
>
...
| Both Sufinama crosswalks now have an AI-classified `match_status`, not a human-confirmed one | The 2026-08-16 first pass read real line-coverage evidence and is not a guess, but `human_confirmed: false` on all 124 records means it hasn't been checked by Rauf yet — treat as strong-prior, not ground truth, especially for training splits. | Rauf reviews `data/annotations/crosswalk_classifications.jsonl` (evidence in `crosswalk_evidence.md`), starting with the one `possible` record and the composite kaafi-44 page; re-run `apply-crosswalk-review` after edits. |
...
| Rafat reference translations are in working files | They are copyrighted and should not enter a public/open corpus or training data. | ~~Resolved for training~~ (Phase 0.2, 2026-08-15): serialized as `reference_translation`, `trainable: false`, and defensively stripped by `export-site`/leak-scanned by `export-training-corpus`. Still relevant if/when public publication is considered. |
| Source PDFs are local | Accidental staging would redistribute scans. | The ignore rule protects `Bulleh Shah/*.pdf`; verify with `git check-ignore -v`. |
| Source variants can leak across train/evaluation splits | 0001 and 0029 already show that one work can have multiple witnesses. | ~~Resolved~~: `src/abshaar/clusters.py` groups witnesses into canonical-work clusters (340 clusters / 356 members as of 2026-08-16); `dataset_gen.py` splits by cluster. |
| 0001 lacks the AI section | It does not conform to the newer template and always serializes an empty AI record. | Needs verification whether this is still true after the Phase 0/1 schema work — check before treating 0001 differently in training data. |
| Shahmukhi readings are unverified | Calligraphic visual transcription includes uncertain lines and possible character errors. | Cross-check selected poems against a reliable edition and use a native/scholarly reviewer. Still open — no edition selected yet. |
| No formal reviews exist | Human corrections are the project's intended core asset. | Still open. The plan (docs/15) substitutes automated gates for this in the interim; a five-poem human slice remains the eventual quality investment. |
| Validation is structural only | Zero errors/warnings can create false confidence about meaning and rights. | Use separate source, linguistic, interpretive, and rights review gates — crosswalk classification (2026-08-16) is one such gate, still `human_confirmed: false`. |
...
| Two new local automation scripts (`thermal_aware_pipeline.sh`, `export_training_bundle.sh`/`import_trained_adapter.sh`) are logic-tested but not yet run against a real model | `selftest`/dry-run paths were verified (thermal check, skip/resume logic, bundle export, import validation); no stage has actually invoked Ollama/mlx-lm yet. | First real run should be watched closely against `docs/17_training_runbook.md`'s troubleshooting table (§7) in case something in the wrapper (nice/taskpolicy, path assumptions) interacts oddly with a live `mlx_lm lora` or `ollama` call. |
| `pmset -g therm` thermal detection is unverified under real load | On this idle M4, no thermal event has ever fired, so the parser has only been exercised against the "no data" branch (treated as not-hot). Whether it correctly parses `CPU_Speed_Limit` once real throttling occurs is unverified. | Watch `training/pipeline_logs/run.log` during the first real long run; if a throttling event fires and the script doesn't pause, `pmset -g therm`'s live field format needs checking directly. |
| Ollama and optional AI packages | Local drafting/RAG/eval commands need them. | ~~Resolved~~: `ai-check` (2026-08-16) confirms Ollama 0.32.13, `qwen3:4b`+`qwen3:8b`, and all 5 optional Python packages installed. |
| The project venv is Python 3.9 while `pyproject.toml` requires >=3.11 | Two 3.9-only breakages already occurred (`write_text(newline=)`, same-quote f-strings). | mlx-lm 0.29.1 installed cleanly on 3.9 (cp39 wheel exists) — the feared venv rebuild was NOT needed. Still worth knowing this venv is off-spec if a future package refuses 3.9. |
| ~~**`torch` does not import in `.venv`**~~ — **RESOLVED, verified 2026-09-24** | This was recorded as the blocker gating the whole RAG half of the pipeline: `build-index`, `sentence_transformers`, `chromadb`, the Phase 2 gate, every baseline run, and `docs/20` §5 augmentation. **It no longer reproduces.** | All six packages were import-tested for real (not `find_spec`) on the unchanged Python 3.9.6 venv: torch 2.8.0 OK, sentence_transformers 5.1.2 OK, chromadb 1.5.9 OK, transformers 4.57.6 OK, ollama OK, mlx_lm 0.29.1 OK. `torch.backends.mps.is_available()` is True. **The venv rebuild that `docs/19` Part 6 and `docs/20` §5 are gated behind is not needed.** Cause of the original failure not diagnosed — it was real on 2026-08-31 and reproduced from `/tmp` then. Do not delete the venv to 'fix' anything without re-testing first. The `ai-check` weakness is unchanged and still worth fixing: `src/abshaar/ollama_client.py:43` uses `find_spec`, so it would have reported healthy throughout. |
| The two off-machine training bundles have never been run on real hardware | Logic, syntax and the prompt-masking algorithm are verified (all 1,038 rows, stub tokenizer), but no CUDA GPU and no second Mac was available on 2026-08-31. First real run may hit environment specifics: exact `cu128` wheel availability for the installed driver, the llama.cpp release asset name pattern, PowerShell execution policy on a locked-down lab machine. | Run `00_bootstrap.ps1` alone first and read `outbox/env_report.json` before starting a long job; each stage is independently runnable and resumable, and `README_WINDOWS.md` §6 / `README_MAC.md` §7 hold the troubleshooting tables. |
| **The model's ceiling is set by the corpus, not by training** — measured 2026-08-31 | 217 of 288 interpretive layers are `created_by: ai` (72 ai_translation, 72 literary_translation, 72 tashreeh, 1 literal_gloss); the only human-authored text is Rafat's 71 reference translations, i.e. someone else's published work. **Human reviews: 0.** So a tuned model largely learns to reproduce Claude's readings of the poems — fluent, plausible, epistemically thin — over 72 of the several hundred kafis attributed to Bulleh Shah. Run 1 demonstrated the same limit mechanically: validation hit its floor in ~200 iterations because 1,038 examples come from ~10 question templates. | More training runs will not move this; the levers, in order of impact, are (1) human-verified interpretive layers starting with the five-poem gold slice, which converts the archive from AI-drafted to scholarly, (2) more poems — the 160 PunjabLibrary items once their text is corrected, (3) confirming the 124 crosswalk classifications, (4) running the paraphrase augmentation that exists but has never been run, for question diversity. Meanwhile what is being built — pipeline, gates, eval harness, rights firewall — is real and will pay off the moment the corpus improves. |
| Website and RAG are not implemented as a public product | Roadmap language may imply more product functionality than exists. | Treat as later milestones after reviewed, rights-safe poems exist. The private RAG (`ask`, retrieval) is implemented but its Chroma index is not yet built. |

## 10. Compact Version

Abshaar is a private, local-first archive and AI-assisted interpretation project shared by Codex and Claude, whose goal is a source-grounded Bulleh Shah expert model. As of 2026-08-16: clean tree on `draft`, in sync with `origin/draft` except possibly the last commit (push is blocked for the assistant by a harness permission classifier — Rauf pushes himself via `! git push origin draft`). 72 poem entries with complete AI-drafted interpretive layers; 76 + 48 Sufinama witnesses now ALL AI-classified against the 72 entries (exact/variant/excerpt/possible/unmatched, with per-record line-coverage evidence) but `human_confirmed: false` throughout — a strong first pass, not yet ground truth; 160 private PunjabLibrary Gurmukhi witnesses; 7 sources, 11 biography claims, 6 timeline events, 13-category inventory. Validation: 0 errors, 0 warnings. 83 tests pass. Hardware: Apple M4 MacBook Air, 16 GB RAM, fanless — the reason Rauf wants model runs deferred to a cooler moment or a bigger machine.

The authoritative next-steps document is `docs/15_bulleh_shah_expert_model_implementation_plan.md` (6-phase pipeline to RAG+LoRA); `docs/17_training_runbook.md` is the step-by-step execution guide. The full local AI stack is ready and verified (Ollama 0.32.13, `qwen3:4b`+`qwen3:8b` pulled, mlx-lm 0.29.1 import-verified, BGE-M3 cached) but NOTHING model-dependent has been run yet — only `build-index` (the Chroma embedding step) remains before the runbook's Phase 2 gate. Two new tools, built 2026-08-16 and logic-tested but not yet run for real, give two ways to proceed once authorized:

1. **This Mac:** `./scripts/thermal_aware_pipeline.sh status` → `touch training/RUN_AUTHORIZED` → `... run`. Background scheduling priority + automatic pause under thermal throttling (`pmset -g therm`), so it doesn't compete with foreground work.
2. **Another Apple Silicon Mac (Path C)** or **Rauf's Windows workstation — Threadripper, 128 GB, NVIDIA RTX 6000-class, which deletes all data at sign-out (Path D)**. Build both bundles with `./scripts/export_training_bundle.sh`; they land in `training/dist/` as ~240 KB ZIPs plus base64 `.txt` twins for email. Each is one resumable command (`RUN_ALL.cmd` / `./run_all.sh`): bootstrap → train → merge/fuse + GGUF → base-vs-tuned generations → pack an outbox with a mandatory copy-off checklist. Prefer D: it is the only path that returns an Ollama-servable Q4_K_M GGUF without a second heavy machine. Bring back the adapter (`scripts/import_trained_adapter.sh`) and the GGUF (`ollama create`). Full detail: `docs/18_workstation_training_runbook.md`.

Two caveats added 2026-08-31: **`torch` in `.venv` does not import** (torch 2.8.0 on Python 3.9), so `build-index` and all RAG/baseline work is blocked until the venv is rebuilt on 3.11/3.12 — `ai-check` misses it because it only `find_spec`s. And off-machine training does not move the acceptance gate: `data/processed/training/eval_baseline.md` still does not exist, and tuned-vs-base is only meaningful once the three baseline rows are measured here.

Rafat's English is copyrighted and must never enter training data (already enforced by a leak scanner); Sufinama/PunjabLibrary material stays private; the repo itself stays private on GitHub. Never overwrite source variants; update affected docs and `OFFLOADING.md` after substantive work.
