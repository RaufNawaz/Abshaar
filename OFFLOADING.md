...
> - If you are on a Harvard Mac Studio instead: `training/portable_bundle/` (or a fresh run of `scripts/export_training_bundle.sh` on the Air first) carries just the gated training dataset; run `./train_standalone.sh` there, then bring the adapter back for `scripts/import_trained_adapter.sh` on the Air. See `training/README.md` §"Path C" and `docs/17_training_runbook.md` §5.
>
...
2. **A Harvard Mac Studio (same mlx toolchain, far more RAM, real fans, no thermal ceiling):** `training/portable_bundle/` already exists — copy it over, run `train_standalone.sh` there (consider Qwen3-8B given the extra RAM, since a `qwen3:8b` baseline already exists), bring the adapter back, `scripts/import_trained_adapter.sh` on this Mac. Full detail: `training/README.md` §"Path C".
