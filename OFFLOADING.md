...
- [ ] Run the acceptance gate that off-machine training does **not** remove: `build-index` → RAG smoke test → the three baseline runs (`eval_baseline.md` does not exist yet) → only then compare the tuned model. Criterion unchanged: tuned+RAG ≥ base+RAG on factual AND tuned ≥ base on honesty.
