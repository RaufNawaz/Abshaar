---
id: bulleh_shah_0009
poet_id: bulleh_shah
title: "Apne sang rulaaeen pyaare (Embrace me, Love)"
work_type: "kafi"
source_ids:
  - source_rafat_selection
rights_status: "verify_before_publication"
review_status: "draft"
---

# Original

اپنے سنگ رلائیں پیارے! اپنے سنگ رلائیں
راہ پُواں تے دھاڑے بیلی؛ جنگل، رکھ، بلائیں
بگھمن پیتے، چت مچتے، بگھنے روکن راہیں
تیرے پار جگا دھر چڑھیا، کنڈے لکھ بلائیں
ہول دِلے توں تھر تھر کنبدا، بیڑا پار لنگھائیں
بلھے شاہ نوں شوہ دا مکھڑا، گھونگھٹ کھول دکھائیں
اپنے سنگ رلائیں پیارے! اپنے سنگ رلائیں

# Script Notes

- Script: Shahmukhi (Perso-Arabic)
- Language spans: Punjabi
- Notes: Vision transcription of Rafat poem 8 (Taufiq Rafat, "A Selection", book p.50), verified against the rendered Nastaliq page image. The calligraphic title اپنے سنگ رلائیں is also the refrain, which opens and closes the kafi. Lines 2-3 are calligraphically difficult and only tentatively read (see Review Notes). Needs native/scholarly verification.

# Transliteration

Apne sang rulaaeen pyaare! apne sang rulaaeen
Raah puaan te dhaare beli; jangal, rakh, balaaeen
Baghman piite, chit machte, baghne rokan raaheen
Tere paar jaga dhar charhya, kande lakh balaaeen
Haul dile tun thar thar kanbda, bera paar langhaaeen
Bulhe Shah noon shoh da mukhra, ghoonghat khol dikhaaeen
Apne sang rulaaeen pyaare! apne sang rulaaeen

# Literal Translation

Embrace me, love, and hold me next your heart.
What sinister jungles stand in my way,
what snakes, what beasts of prey,
and on the other side what spirits start.
Reach out your hand to me so fear may cease.
One glimpse of you and I shall be at peace.

Embrace me, love, and hold me next your heart.

_Reference translation by Taufiq Rafat, "Bulleh Shah: A Selection" (Vanguard, 1982), p.51. COPYRIGHTED — private reference only; do NOT publish or use for training._

# AI Translation

Merge me into Yourself, beloved! Merge me into Yourself.
The roads are beset with raiders, comrade — jungle, thicket, terrors;
[wild beasts lie in wait,] they block the paths;
on the far side of You the ground rears up, with a hundred thousand thorns and dangers.
With dread the heart trembles and shakes — ferry my boat across.
Show Bulleh Shah the Beloved's face; lift the veil and reveal it.
Merge me into Yourself, beloved! Merge me into Yourself.

_AI-drafted (Claude) from the Urdu original; needs review. Lines 2-3 rest on tentative readings of obscure words (see Review Notes)._

# Literary Translation

Merge me into yourself, beloved — merge me into yourself.
The road is beset with raiders, my friend: jungle, thicket, lurking terrors;
the beasts lie in wait and block the paths,
and beyond you the ground rears up, a hundred thousand thorns and dangers.
The heart shakes and shudders with dread — ferry my boat across.
Lift the veil and show Bulleh Shah the Beloved's face.
Merge me into yourself, beloved — merge me into yourself.

_AI-drafted (Claude) literary rendering from the Urdu original — original wording, NOT derived from Rafat; lines 2–3 rest on tentative readings. Draft for your review; replace with your own._

# Tashreeh

The refrain sets the poem's goal as union in its strongest form: rulāīṅ, "merge me into yourself" — not nearness to the Beloved but absorption into the Beloved, the language of fanāʾ, the annihilation of the separate self in the divine. Everything framed between the two statements of the refrain is the obstacle-course that lies between the seeker and that merging, and the method is to render the spiritual path as a literal perilous journey — the old topos of the ṭarīqa, the "road," as a dangerous crossing through wild country.

The danger-catalogue — raiders on the road, jungle and thicket, wild beasts, "a hundred thousand thorns," terrors on the far side — allegorizes the perils of the path: the highwaymen (dhāṛe) are the passions and the nafs that ambush the traveller, and the beasts and thorns are the trials and the "hundred thousand" veils that stand between the soul and God. (Several of these words are calligraphically obscure and only tentatively read, so the exact inventory of dangers is provisional — but the structure, a gauntlet barring the way to union, is unmistakable.) The trembling heart that "shakes and shudders with dread" is the seeker's fear before the crossing, and the plea "ferry my boat across" (beṛā pār langhāīṅ) reaches for Bulleh Shah's recurrent image of grace as a ferrying: the traveller cannot cross by his own effort; he must be carried.

The climax joins two images: "ferry my boat across" and "lift the veil (ghūnghaṭ) and show me the Beloved's face (mukhṛā)." The ghūnghaṭ, the bride's veil, once again casts the soul as feminine and names the goal as dīdār, the unveiled vision of the Beloved. Read as a sequence, the poem lays out the classic mystical arc in miniature — the perilous road, the ferrying by grace, the unveiling, and the merging of the refrain — from separation through trial to union and annihilation; and the "boat" and "ferry" set the whole drama on the Punjabi river, where to cross the water is to cross out of the self.

_AI-drafted (Claude) interpretive note; needs your review._

# Key Terms

- rulāīṅ: "merge/mingle me" — absorption into the Beloved, the vocabulary of fanāʾ; do_not_flatten_to a simple embrace.
- dhāṛe: raiders/highwaymen on the road — the passions and the nafs that ambush the traveller; do_not_flatten_to ordinary bandits.
- beṛā pār langhāīṅ: "ferry my boat across" — grace carrying the soul over water it cannot cross alone; do_not_flatten_to literal boating.
- ghūnghaṭ: the bride's veil, whose lifting is the unveiling (dīdār) of the Beloved's face; do_not_flatten_to a piece of cloth.
- shoh: the Beloved/Lord, a Punjabi endearment for the divine spouse; do_not_flatten_to a human lover only.

# Themes

- union_with_the_beloved
- perilous_path
- annihilation_fana
- grace_as_ferrying
- unveiling_didar

# Source Notes

- Source ID: source_rafat_selection
- Source title: Bulleh Shah: A Selection, Rendered into English Verse (Taufiq Rafat, 1982)
- URL or bibliographic reference: Vanguard Books, Lahore, 1982 — Rafat poem 8 "Embrace me, Love", English book p.51 / Urdu original p.50
- Rights status: Original Punjabi verse public domain (poet d. 1757); Rafat's English translation copyrighted (recorded here as private reference only)
- Can this be published? no
- Can this be used for model training? no

# Review Notes

- Questions: Verify the Nastaliq transcription against a print Kulliyat, especially the danger-catalogue in lines 1-3.
- Uncertainties: Line 2 "بگھمن پیتے، چت مچتے، بگھنے روکن راہیں" is a tentative reading (obscure words, likely wild beasts blocking the path); line 3 "تیرے پار جگا دھر چڑھیا" and "راہ پُواں تے دھاڑے بیلی" (line 1) are best readings; short vowels are best readings.
- Alternate readings: (pending)
- Reviewer: (pending - rauf)
- Date: 2026-07-06 (vision transcription verified against page image; transliteration + AI translation by assistant; Rafat English recorded as gated reference; interpretive layers pending)
