---
id: bulleh_shah_0012
poet_id: bulleh_shah
title: "Amaan baabe di bhalyaai (Heritage)"
work_type: "kafi"
source_ids:
  - source_rafat_selection
rights_status: "verify_before_publication"
review_status: "draft"
---

# Original

اَماں بابے دی بھلیائی، اوہ ہُن کِتّ اساڈے آئی
اَماں بابا چور دُھمراں دے، پُتر دی وڈیائی
دَلائے اُتّوں گُت بگُتّی، گھر گھر پئی لڑائی
اَساں قضیّے تائیں جاہلے، جد کنک اُنھاں ٹُرکائی
کھائے خیراتے پچاہیے جمّا، اُلٹی دستک لائی
اَماں بابے دی بھلیائی، اوہ ہُن کِتّ اساڈے آئی

# Script Notes

- Script: Shahmukhi (Perso-Arabic)
- Language spans: Punjabi (colloquial/idiomatic)
- Notes: Vision transcription of Rafat poem 11 (Taufiq Rafat, "A Selection", book p.58), verified against the rendered Nastaliq page image. The calligraphic title اَماں بابے دی بھلیائی is also the refrain, opening and closing the kafi. Several interior words are colloquial/archaic and hard to read with confidence (see Review Notes) — this entry needs careful native/scholarly verification more than most.

# Transliteration

Amān bābe di bhalyāi, oh hun kitt asāde āyi
Amān bāba chor dhumrān de, putar di vadyāi
Dalāe utton gut baguti, ghar ghar payi larāi
Asān qaziye tāīn jāhle, jad kanak unhān turkāi
Khāe khairāte pachāhiye jamma, ulti dastak lāi
Amān bābe di bhalyāi, oh hun kitt asāde āyi

# Literal Translation

What parents sowed, the children reap.
Who's the benefactor? Who the thief?
When the corn came in, the knives came out;
brother fought brother for every sheaf.
One sinned, another bears the grief.
What parents sowed, the children reap.

_Reference translation by Taufiq Rafat, "Bulleh Shah: A Selection" (Vanguard, 1982), p.59. COPYRIGHTED — private reference only; do NOT publish or use for training._

# AI Translation

The "goodness" bequeathed by mother and father — where has it reached us now?
Mother and father, thieves of ill repute — and this the son's "renown."
From the courtyards, scuffles and hair-pulling; house upon house fell to quarrelling.
We were dragged into lawsuits the moment they made off with the wheat.
They ate the funeral alms... and the knocking came back reversed.
The "goodness" bequeathed by mother and father — where has it reached us now?

_AI-drafted (Claude) from the Urdu original; needs review. Interior lines rest on tentative readings of colloquial/obscure words (see Review Notes)._

# Literary Translation

What "goodness" did mother and father leave us — and where has it brought us now?
Mother and father, thieves of ill repute — and this they call the son's renown.
Courtyard scuffles, hair pulled house to house, quarrels breaking out door to door;
we were hauled into the lawsuits the moment they made off with the wheat.
They ate the funeral alms — and the knock came back reversed upon them.
What "goodness" did mother and father leave us — and where has it brought us now?

_AI-drafted (Claude) literary rendering from the Urdu original — original wording, NOT derived from Rafat; interior lines rest on tentative readings. Draft for your review; replace with your own._

# Tashreeh

This is Bulleh Shah in a rare satirical-social register, and the engine is irony. The refrain asks what the "goodness/benefit" (bhalyāī) bequeathed by one's parents has actually amounted to — "where has it reached us now?" — and the answer, line by line, is not virtue but a legacy of theft, ill repute, brawling, and litigation. The poem inverts the pious commonplace of honouring the inheritance of one's forebears: what is really handed down, it says, is strife. Set against Bulleh Shah's wider work, the target is the whole notion of inherited worth — the same suspicion of lineage and zāt (caste, family honour) that drives "Chal Bulleh" (0008). The only inheritance worth having is not the ancestors' name or property but a purified heart, and the bitter comedy is that the actual bequest is quarrels over wheat and misappropriated alms.

The concrete images root the satire in the everyday economic life of a Punjabi village: hair-pulling scuffles from courtyard to courtyard, lawsuits (qaziye) erupting the instant the harvest is divided, and the sharp line about eating the khairāt — funeral or charity alms — with "the knock coming back reversed," a folk image of a curse rebounding or of alms turning to affliction. The critique is at once social and spiritual: greed and litigiousness are the true "heritage," and the misused alms suggest that even religious observance has been bent into another instrument of grasping.

A prominent caveat: this entry carries HIGH transcription uncertainty across its interior lines (the phrase rendered "thieves of ill repute," the wheat/lawsuit line, and the "funeral alms / reversed knock" line are all tentative), so the reading above is provisional and a priority for native/scholarly verification. The general satirical thrust — ironic inheritance, strife over property, corrupted charity — is secure; the specific glosses are not.

_AI-drafted (Claude) interpretive note; needs your review._

# Key Terms

- bhalyāī: "goodness/benefit" — used ironically for the dubious inheritance of one's forebears; do_not_flatten_to sincere praise.
- gut-bagutī: a hair-pulling brawl — the household strife that is the real legacy; do_not_flatten_to a mild quarrel.
- qaziya: a dispute or lawsuit — here the litigation over property that consumes the heirs; do_not_flatten_to an abstract "issue."
- khairāt: alms, especially funeral or charity alms — misappropriated here, marking the corruption of religious giving; do_not_flatten_to ordinary charity.

# Themes

- inherited_strife
- social_satire
- critique_of_lineage
- corruption_of_charity
- irony

# Source Notes

- Source ID: source_rafat_selection
- Source title: Bulleh Shah: A Selection, Rendered into English Verse (Taufiq Rafat, 1982)
- URL or bibliographic reference: Vanguard Books, Lahore, 1982 — Rafat poem 11 "Heritage", English book p.59 / Urdu original p.58
- Rights status: Original Punjabi verse public domain (poet d. 1757); Rafat's English translation copyrighted (recorded here as private reference only)
- Can this be published? no
- Can this be used for model training? no

# Review Notes

- Questions: Verify the Nastaliq transcription against a print Kulliyat — this poem especially.
- Uncertainties: HIGH on interior lines. Line 1 "چور دُھمراں دے" (dhumraan uncertain); line 3 "قضیّے تائیں جاہلے، جد کنک اُنھاں ٹُرکائی" (possibly "جد تک", "until"; whole line tentative); line 4 "پچاہیے جمّا، اُلٹی دستک لائی" (tentative). Refrain "اوہ ہُن کِتّ اساڈے آئی" ("کِتّ" possibly "کِتھّے"/where). Short vowels are best readings.
- Alternate readings: (pending)
- Reviewer: (pending - rauf)
- Date: 2026-07-06 (vision transcription verified against page image; transliteration + AI translation by assistant; Rafat English recorded as gated reference; interpretive layers pending)
